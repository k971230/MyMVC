...
function goEditMyInfo(userid,ctx_Path) {
	...
	// 나의정보 수정하기 팝업창 띄우기
	const url = `${ctx_Path}/member/memberEdit.up?userid=${userid}`; //JSP 안에서 스크립트를 작성할때는 이렇게 쓰면 안된다.(requestScope처럼 저장소로 인식됨) 변수로 인식하게 하기위해 $앞에 역슬래시를 써서 표현해줘야한다.
	// 또는
	//const url = ctx_Path+"/member/memberEdit.up?userid="+userid;
	
	// 너비 800, 높이680 인 팝업창을 화면 가운데 위치시키기
	const width = 800;
	const height = 680;
...
function goCoinPurchaseTypeChoice(userid,ctx_Path) {
	...
	// 너비 650, 높이570 인 팝업창을 화면 가운데 위치시키기
	const width = 650;
	const height = 570;
...
function goCoinPurchaseEnd(ctx_Path, coinmoney, userid) {
	...
	// 너비 1000, 높이600 인 팝업창을 화면 가운데 위치시키기
	const width = 1000;
	const height = 600;
